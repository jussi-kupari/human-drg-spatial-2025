...
p1 <- 
  ...
  geom_hline(yintercept = mean(md@meta.data$nFeature_RNA), 
             linewidth = 3, 
             color = "red", 
             alpha = 0.5) +
  ggtitle(str_glue("mean: {round(mean(md@meta.data$nFeature_RNA))}"))
...
p2 <- QC_Plots_UMIs(seurat_object = md, group.by = "treatment") +
  geom_hline(yintercept = mean(md@meta.data$nCount_RNA), 
             ...
             color = "red", 
             alpha = 0.5) +
  ggtitle(str_glue("mean: {round(mean(md@meta.data$nCount_RNA))}"))
...
p3 <- QC_Plots_Mito(seurat_object = md, group.by = "treatment") +
  geom_hline(yintercept = mean(md@meta.data$percent_mito), 
             ...
             color = "red", 
             alpha = 0.5) +
  ggtitle(str_glue("mean: {round(mean(md@meta.data$percent_mito), digits = 1)}"))
...
p4 <- QC_Plots_Complexity(seurat_object = md, group.by = "treatment") +
  geom_hline(yintercept = mean(md@meta.data$log10GenesPerUMI), 
             ...
             color = "red", 
             alpha = 0.5) +
  ggtitle(str_glue("mean: {round(mean(md@meta.data$log10GenesPerUMI), digits = 2)}"))

p5 <- patchwork::wrap_plots(p1, p2, p3, p4, ncol = 4)

p6 <- QC_Plots_Genes(seurat_object = kathe_neurons, group.by = "global_cell_type") +
  geom_hline(yintercept = mean(kathe_neurons@meta.data$nFeature_RNA), 
             linewidth = 3, 
             color = "red", 
             alpha = 0.5)  +
  ggtitle(str_glue("mean: {round(mean(kathe_neurons@meta.data$nFeature_RNA))}"))
...
p7 <- QC_Plots_UMIs(seurat_object = kathe_neurons, group.by = "global_cell_type") +
  geom_hline(yintercept = mean(kathe_neurons@meta.data$nCount_RNA), 
             ...
             color = "red", 
             alpha = 0.5) +
  ggtitle(str_glue("mean: {round(mean(kathe_neurons@meta.data$nCount_RNA))}"))

p8 <- QC_Plots_Mito(seurat_object = kathe_neurons, group.by = "global_cell_type") +
  geom_hline(yintercept = mean(kathe_neurons@meta.data$percent_mito), 
             linewidth = 3, 
             color = "red", 
             alpha = 0.5) +
  ggtitle(str_glue("mean: {round(mean(kathe_neurons@meta.data$percent_mito), digits = 1)}"))

p9 <- QC_Plots_Complexity(seurat_object = kathe_neurons, group.by = "global_cell_type") +
  geom_hline(yintercept = mean(kathe_neurons@meta.data$log10GenesPerUMI), 
             linewidth = 3, 
             color = "red", 
             alpha = 0.5) +
  ggtitle(str_glue("mean: {round(mean(kathe_neurons@meta.data$log10GenesPerUMI), digits = 2)}"))
